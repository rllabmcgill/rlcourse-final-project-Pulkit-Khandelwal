"""
RBF kernel impemented alongwith other non-linear features.
GridWorld Environment: Credits to M. Alger for some part of this code.
...
class Gridworld(object):
    ...
    def feature_vector(self, i, feature_map="ident"):
        ...
        if feature_map == "kernel":
            f = np.zeros(self.grid_size)
            x, y = i % self.grid_size, i // self.grid_size
            f[x] += math.exp(float((x - 2.5)**2) / (-1*2))
            f[y] += math.exp(float((y - 2.5)**2) / (-1*2))
            return f        
        if feature_map == "coordinate":
            ...
            f[y] += 1
            return f
        if feature_map == "exponential":
            f = np.zeros(self.grid_size)
            x, y = i % self.grid_size, i // self.grid_size
            f[x] += math.exp(x)
            f[y] += math.exp(y)
            return f     
        if feature_map == "sigmoid":
            f = np.zeros(self.grid_size)
            x, y = i % self.grid_size, i // self.grid_size
            f[x] += float(1)/ (1+math.exp(-x))
            f[y] += float(1)/ (1+math.exp(-y))
            return f                   
